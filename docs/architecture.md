...
`codex-midi` has three replaceable boundaries:
...
ATOM. It consumes and produces raw HID reports and exposes normalized controller
...
Node subprocess for the CommonJS preload compatibility coverage.

Generic tests cover shared MIDI, Project2077, socket, dormancy, and preload
contracts. The bundled ATOM additionally has a black-box regression through
